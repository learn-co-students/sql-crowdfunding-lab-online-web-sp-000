...
"SELECT projects.title, SUM(pledges.amount)-projects.funding_goal AS INTEGER FROM projects JOIN pledges ON projects.id = pledges.project_id  GROUP BY projects.title HAVING SUM(pledges.amount)-projects.funding_goal >= 0"
end
