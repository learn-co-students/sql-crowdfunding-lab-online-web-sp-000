...
"SELECT projects.title, SUM(pledges.amount)-projects.funding_goal AS amount_over_goal FROM projects JOIN pledges ON projects.id = pledges.project_id  GROUP BY projects.title ORDER BY amount_over_goal DESC LIMIT 2"
end
